...
// Special case: if destination is rooms.diez.gallery, append ?t=trackingId
// so the viewing room can continue the engagement tracking chain
export function wrapLinksWithTracking(html, trackingId, campaignId, recipientEmail) {
  ...
  return html.replace(
    ...
    (match, url) => {
      ...
      // If the destination is the viewing room, propagate the tracking ID
      let destination = url;
      if (url.includes('rooms.diez.gallery')) {
        const separator = url.includes('?') ? '&' : '?';
        destination = `${url}${separator}t=${trackingId}`;
      }
