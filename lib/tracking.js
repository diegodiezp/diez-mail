...
// Decode and verify. Returns null if tampered or unsigned-and-off-domain.
export function decodeTrackingData(encoded) {
  try {
    const json = Buffer.from(encoded, 'base64url').toString('utf-8');
    const data = JSON.parse(json);

    // Legacy links sent before signing was introduced carry no `sig`.
    // Honor them, but only ever toward gallery-owned domains, so an
    // unsigned payload can never be used as an open redirect elsewhere.
    if (!data.sig) {
      const ALLOWED_HOSTS = ['diez.gallery', 'rooms.diez.gallery'];
      try {
        const host = data.url ? new URL(data.url).hostname : null;
        if (host && ALLOWED_HOSTS.includes(host)) return data;
      } catch {}
      return null;
    }
